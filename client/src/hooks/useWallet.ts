...
export function useWallet(): UseWalletReturn {
    ...
    const init = async () => {
      ...
      if (accounts.length > 0) {
        ...
        if (currentChainId !== DEFAULT_CHAIN_ID) {
          console.log(`Wallet connected to chainId ${currentChainId}, our app needs ${DEFAULT_CHAIN_ID}`);
    ...
    try {
      ...
      if (newProvider && accounts.length > 0) {
        ...
        if (newChainId && newChainId !== DEFAULT_CHAIN_ID) {
          console.log(`Connected to chain ${newChainId}, but active chain is ${DEFAULT_CHAIN_ID}`);
  ...
  // Switch network
  const switchNetwork = useCallback(async (newChainId: string = DEFAULT_CHAIN_ID): Promise<boolean> => {
